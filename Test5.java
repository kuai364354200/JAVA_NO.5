...
public class Test5 {
	public static void main(String[] args) {
	        ...
	        try {
	            FileReader fileReader = new FileReader("C:\\Users\\kuai3\\eclipse-workspace\\B.txt");//读取文件
	            BufferedReader bufferedReader = new BufferedReader(fileReader);//把文件存到缓冲区
	            Writer writer = new FileWriter(new File("C:\\Users\\kuai3\\eclipse-workspace\\A.txt"));//将缓冲区文件写入A文件中
	            String str = bufferedReader.readLine();
	            String regex = "(.{7})";
	            str = str.replaceAll(regex, "$1，");
	            StringBuffer x = new StringBuffer(str);
	            for (int  i = 15; i <1024; i = i + 17) {
	                x.replace(i, i + 1, "。\n");
	            }
